...
//
stock CarDoors(playerid, carid, is_doors = true)
{
    if (GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
    {
        if (!IsAPlane(carid) && !IsABoat(carid) && !IsABike(carid) && AutoSaloon[playerid] != true)
    ...
    GetVehicleParamsEx(carid, engine, lights, alarm, doors, bonnet, boot, objective);
    if (doors) SetVehicleParamsEx(carid, engine, lights, alarm, false, bonnet, boot, objective);
    else if (!doors) SetVehicleParamsEx(carid, engine, lights, alarm, true, bonnet, boot, objective);
}