public OnPlayerRequestClass(playerid, classid)
{
	...
	SendMes(playerid, COLOR_BLUE, "REQUEST: PLAYERID - %i, CLASSID - %i", playerid, classid);
	...
	mysql_function_query(DATABASE, QUERY, true, "_MySQL_QUERY", "iis", 4, playerid, "");
	mysql_format(DATABASE, QUERY, 256, "SELECT `Name` FROM `"TABLE_ACCOUNTS"` WHERE `Name` = '%e'", Name(playerid));
	mysql_function_query(DATABASE, QUERY, true, "_MySQL_QUERY", "iis", 5, playerid, "");
